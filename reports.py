...
def get_selling_avg(file_name):
	...
	return round(sold_count / games_count, rounding_digit)

def count_longest_title(file_name):
	list_of_games = get_list_from_file(file_name)
	return len(max(row[0] for row in list_of_games))
...
def get_date_avg(file_name):
	...
	games_count = count_games(file_name)
	list_of_games = get_list_from_file(file_name)
	dates_added = sum(int(row[DATE_INDEX]) for row in list_of_games)
	return round(dates_added / games_count)
